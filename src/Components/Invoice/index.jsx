...
const Invoice = () => {
    ...
    const { mutateAsync: generateInvoicePdf, } = trpc.employee.generatePDFinvoice.useMutation();
    ...
    const isInvoiceEmpty = invoiceData.items.length === 0;
    const isShowPrice = isInvoiceEmpty || invoiceData.items.some((item) => item.Price > 0);
    const isShowHours = isInvoiceEmpty || invoiceData.items.some((item) => item.Hours > 0);
    ...
    return (
        <div>
            ...
            <div className='p-8 bg-gray-50'>
                <Layout hasSider >
                    <Sider style={siderStyle} className='rounded-[12px] !bg-gray-50' theme="light" width={400}>
                        <div className='flex flex-col gap-[8px]'>
                            <div className="p-3 bg-white border border-gray-200 rounded-[12px]">
                            ...
                            <div className="p-3 bg-white border border-gray-200 rounded-[12px]">
                                <h2 className="text-lg font-semibold mb-4">Tickets Table</h2>
                            ...
                            <div className="p-3 bg-white border border-gray-200 rounded-[12px]">
                                ...
                                <p className='mt-2'>Receiver Emails (coma separated)</p>
                                <Input
                    ...
                    <Layout>
                        <Content style={{ overflow: 'initial' }} className='bg-gray-50 pt-2'>
                            <>
                                <div id="invoice" className="w-[817px] mx-auto bg-white rounded-[12px] p-[32px] border border-gray-200">
                                    {/* Header */}
                                    ...
                                    <table className="w-full mt-8 border border-gray-300 border-collapse">
                                        <thead>
                                            <tr className="border-b-[0.7px] border-[#E6E9EB] h-[40px] text-left font-semibold">
                                                ...
                                                {isShowHours && <th className="px-6 py-3 w-[13.7%]">Hours</th>}
                                                {isShowPrice && <th className="px-6 py-3 w-[19.2%]">Price</th>}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {invoiceData.items.map((item, index) => (
                                                <tr key={index} className="border-b border-gray-300 h-[40px]">
                                                    ...
                                                    {isShowHours && <td className="px-6 ">{item.Hours}</td>}
                                                    {isShowPrice && <td className="px-6">${item.Price}</td>}
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                    ...
                                    <div>
                                        <p className="font-poppins font-normal mt-[70px] text-[14px] leading-[15.7px] tracking-[0.07px]">
                                            <span className='font-bold'>Note:</span> This invoice is system-generated and does not require a signature or official stamp.
                                        </p>
                                    </div>
                                    <div className="w-full border-t border-[#656565] opacity-20 my-4"></div>
                                    <div className="flex space-x-4">
                                        <div className="flex-1">Business Centre, Sharjah Publishing City<br /> Free Zone, Sharjah, United Arab Emirates</div>
                                        <div className="flex-1">+1 435-328-2694<br />
